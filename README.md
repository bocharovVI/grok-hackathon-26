...
Для `grok-4.6` используется `XAI_REASONING_EFFORT=low`, чтобы сократить ожидание
при извлечении данных. Можно выбрать `medium` или `high`; это может увеличить
время ответа. Для модели без поддержки этого параметра задайте пустое значение.
`XAI_TIMEOUT_SECONDS=300` задаёт таймаут чтения ответа, подключение ограничено
10 секундами, отправка — 60 секундами. Если в Render уже задано `120`, обновите
значение в Environment backend и передеплойте сервис. В логах `Grok request started`,
`Grok response received` и `Grok timeout` видны модель, длительность и тип таймаута;
содержимое документа и ключ API не выводятся. Увеличение таймаута не гарантирует
успеха при недоступности x.ai или медленной обработке большого документа.
...
10 страниц PDF, изображения до 20 мегапикселей, таймаут чтения ответа Grok 300 секунд); нет фоновой очереди и автоматических
